...
from models.model import BTDRF
from utils import *
...
def OctreeRender_trilinear_fast(rays, tensorf, chunk=4096, N_samples=-1, ndc_ray=False,white_bg=True, is_train=False,
                                device='cuda'):
    rgbs, alphas, depth_maps, weights, uncertainties, normals = [], [], [], [], [], []
    N_rays_all = rays.shape[0]
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        rays_chunk = rays[chunk_idx * chunk:(chunk_idx + 1) * chunk].to(device)

        output = tensorf(rays_chunk, is_train=is_train, white_bg=white_bg, ndc_ray=ndc_ray, N_samples=N_samples)

        rgbs.append(output['rgb_map'])
        depth_maps.append(output['depth_map'])

    return torch.cat(rgbs), None, torch.cat(depth_maps), None, output


@torch.no_grad()
def evaluation(test_dataset, tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
               white_bg=False, ndc_ray=False, contract_ray=False, compute_extra_metrics=True, device='cuda'):
